...
from transit import gettransitdepartures, get_transit_config_data
...
class transit:
    transit_api_key : str
    transit_stop1 : str
    transit_stop1_name : str
    transit_stop1_number : str 
    transit_stop1_type : str
    transit_stop2 : str
    transit_stop2_name : str
    transit_stop2_number : str
    transit_stop2_type : str
...
def MorningDash():
    ...
    imageB = Image.new('L', (screen.width, screen.height), 255)  # 255: clear the frame
    imageR = Image.new('L', (screen.width, screen.height), 255)  # 255: clear the frame
    ...
    if dashboard.show_transit == 1:
        applog("Dashboard","Transit feature : ON")
        applog("Transit feature","Getting config data...")
        transit_config = get_transit_config_data("transit.ini")
        transit.transit_api_key = transit_config['transit_api_key-id']
        transit.transit_stop1 = transit_config['Stop_1']
        transit.transit_stop1_name = transit_config['Stop_1_name']
        transit.transit_stop1_number = transit_config['Stop_1_number']
        transit.transit_stop1_type = transit_config['Stop_1_type']
        transit.transit_stop2 = transit_config['Stop_2']
        transit.transit_stop2_name = transit_config['Stop_2_name']
        transit.transit_stop2_number = transit_config['Stop_2_number']
        transit.transit_stop2_type = transit_config['Stop_2_type']
        applog("Transit feature","Got config data for transit")

        applog("Transit feature","Getting bus times...")

        # Get bus times
        transit_times1 = []
        transit_times2 = []
        transit_times1 = gettransitdepartures(datetime.now(), transit.transit_api_key, transit.transit_stop1)
        if len(transit_times1) == 0:
            applog("Transit feature","No departure times found for stop 1, using default values")
            transit_times1 = ["--:--", "--:--", "--:--"]
        else:
            applog("Transit feature","Got bus times for stop 1"+ str(transit_times1))
        transit_times2 = gettransitdepartures(datetime.now(), transit.transit_api_key, transit.transit_stop2)
        if len(transit_times2) == 0:
            applog("Transit feature","No departure times found for stop 2, using default values")
            transit_times2 = ["--:--", "--:--", "--:--"]
        else:
            applog("Transit feature","Got bus times for stop 2"+ str(transit_times2))

        if transit.transit_stop1_type == "BUS":
            transit_1_icon = Image.open(os.path.join(picdir, "transit_bus_smile.png"))
        elif transit.transit_stop1_type == "METRO":
            transit_1_icon = Image.open(os.path.join(picdir, "transit_metro_train.png"))
        elif transit.transit_stop1_type == "TRAIN":
            transit_1_icon = Image.open(os.path.join(picdir, "transit_train.png"))
        else:
            applog("Transit feature","Unknown transit type for stop 1, using generic icon")
            transit_1_icon = Image.open(os.path.join(picdir, "transit_generic.png"))

        if transit.transit_stop2_type == "BUS":
            transit_2_icon = Image.open(os.path.join(picdir, "transit_bus_smile.png"))
        elif transit.transit_stop2_type == "METRO":
            transit_2_icon = Image.open(os.path.join(picdir, "transit_metro_train.png"))
        elif transit.transit_stop2_type == "TRAIN":
            transit_2_icon = Image.open(os.path.join(picdir, "transit_train.png"))
        else:
            applog("Transit feature","Unknown transit type for stop 2, using generic icon")
            transit_2_icon = Image.open(os.path.join(picdir, "transit_generic.png"))
        applog("Transit feature","Icons have been assigned for stops")
        
        # Draw bus times
        applog("Transit feature","Drawing bus times...")
        transitIconX = 10
        transitIconY = screen_y + 10
        transitIconBottom = int((transitIconY + transit_1_icon.size[1]))
        transitName_t_size = draw_black.textbbox((0,0),transit.transit_stop1_name, font=font.SF_TransitName)
        transitName_t_w = transitName_t_size[2] + transitName_t_size[0]
        transitName_t_h = transitName_t_size[3]
        transitID_t_size = draw_black.textbbox((0,0),transit.transit_stop1_number, font=font.SFTransitID)
        transitID_t_w = transitID_t_size[2] + transitID_t_size[0]
        transitID_t_h = transitID_t_size[3]

        
        transiticon_nameX = 10
        transiticon_nameY = transitIconBottom + 2
        transitName_w_middle = transiticon_nameX + int(transitName_t_w/2)

        transitIconX = transitName_w_middle - int(transit_1_icon.size[0]/2)
        transitIconY = screen_y + 10

        tranistIconMiddle = int(transitIconX + (transit_1_icon.size[0]/2))

        transiticon_idX = int(tranistIconMiddle - int(transitID_t_w/2))
        transiticon_idY = transitIconY + int(transitName_t_h/2)


        imageB.paste(transit_1_icon, (transitIconX, transitIconY), transit_1_icon)
        draw_black.text((transiticon_nameX, transiticon_nameY), transit.transit_stop1_name, font=font.SF_TransitName, fill=black)
        draw_black.text((transiticon_idX, transiticon_idY), transit.transit_stop1_number, font=font.SFTransitID, fill=black)

        applog("Transit feature","Stop 1 icon and text drawn")
        transit_times1_x = transitIconX + transit_1_icon.size[1] + 15
        transit_times1_y = (transitIconY -5)
        bus_time_t_size = draw_black.textbbox((0,0),"00:00", font=font.SFTransitTime)
        bus_time_t_w = bus_time_t_size[2] + bus_time_t_size[0]
        bus_time_t_h = bus_time_t_size[3]
    
        # Draw bus times for stop 1
        for i in transit_times1 :
            draw_black.text((transit_times1_x,transit_times1_y), i, font=font.SFTransitTime, fill=black)
            transit_times1_y = transit_times1_y + bus_time_t_h + 2
        
        # Draw bus times
        applog("Transit feature","Drawing bus times...")
        transitIconX = transit_times1_x + bus_time_t_w + 20
        transitIconY = screen_y + 10
        transitIconBottom = int((transitIconY + transit_2_icon.size[1]))
        transitName_t_size = draw_black.textbbox((0,0),transit.transit_stop2_name, font=font.SF_TransitName)
        transitName_t_w = transitName_t_size[2] + transitName_t_size[0]
        transitName_t_h = transitName_t_size[3]
        transitID_t_size = draw_black.textbbox((0,0),transit.transit_stop2_number, font=font.SFTransitID)
        transitID_t_w = transitID_t_size[2] + transitID_t_size[0]
        transitID_t_h = transitID_t_size[3]

        
        transiticon_nameX = transitIconX
        transiticon_nameY = transitIconBottom + 2
        transitName_w_middle = transiticon_nameX + int(transitName_t_w/2)
        ...
        imageB.paste(transit_2_icon, (transitIconX, transitIconY), transit_2_icon)
        draw_black.text((transiticon_nameX, transiticon_nameY), transit.transit_stop2_name, font=font.SF_TransitName, fill=black)
        draw_black.text((transiticon_idX, transiticon_idY), transit.transit_stop2_number, font=font.SFTransitID, fill=black)

        applog("Transit feature","Stop 1 icon and text drawn")
        transit_times2_x = transitIconX + transit_2_icon.size[1] + 15
        transit_times2_y = (transitIconY -5)
        bus_time_t_size = draw_black.textbbox((0,0),"00:00", font=font.SFTransitTime)
        bus_time_t_w = bus_time_t_size[2] + bus_time_t_size[0]
        bus_time_t_h = bus_time_t_size[3]
    
        # Draw bus times for stop 1
        for i in transit_times2 :
            draw_black.text((transit_times2_x,transit_times2_y), i, font=font.SFTransitTime, fill=black)
            transit_times2_y = transit_times2_y + bus_time_t_h + 2

        screen_y = transit_times2_y #+ bus_time_t_h
        ...
        applog("Dashboard","Transit feature : OFF")
        transit_times1 = []
        transit_times2 = []
    ...
    # MASK Section
    if dashboard.show_weather ==1 and dashboard.show_mask_warning == 1:
    ...
    imageB.save("MorningDash_InkFrame_B.png", format='png')
...
def DayDash():
 ...
 #   applog("dashboard.show_todo",str(dashboard.show_todo))
    ...
    if dashboard.show_quote == 1 and can_show_quote:
        ...
        dashboard.quote.quote_text = "This is a test quote to see how the text is sliced and displayed on the screen. It should be long enough to test the slicing functionality. Just a few more words to see."
        dashboard.quote.quote_author = "Dashboard Ai Error ;("
        qll = len(dashboard.quote.quote_text)
    ...
    if dashboard.show_weather ==1 and dashboard.show_mask_warning == 1:
        ...
        if today_aqi.aqi_value > 100:
            ...
            shape = [(0, screen_y), (screen.width, int(mask_icon.height + screen_y + 10))]
            # create rectangle image 
            draw_black.rectangle(shape, fill = white)
            draw_red.rectangle(shape, fill = white) 
    ...
    imageB.save("DayDash_InkFrame_B.png", format='png')
...
def get_dashboard_config_data(file_path:str):
    ...
    data['show_mask_warning-id'] = parser.get("feature-config", "show_mask_warning")
    ...
    if data['show_mask_warning-id'] == "TRUE":
        dashboard.show_mask_warning = 1
    else : 
        dashboard.show_mask_warning = 0 
    ...
    if data['shutdown-after-run-id'] == "EXIT":
        dashboard.shutdown_after_run = 2
    else : 
        dashboard.shutdown_after_run  = 0
...
def welcome_screen(delay_time_in_sec):
    ...
    if battlevel.level !=-1: #Checking if battery level can be goten -1 means error
        ...
        bty = (screen.height - 5) - int(batt_icon.height)
        imageB.paste(batt_icon, (btx,bty), batt_icon)
    imageB.save("InkFrame_B.png", format='png')
...
def main():
    ...
    while True :
        ...
        if dashboard.shutdown_after_run == 1:
            gotosleep()
        elif dashboard.shutdown_after_run == 2:
            applog("HomeInkFrame","Exiting the program...")
            exit()
        else:
