...
def get_garbage_config_data(file_path:str):
    ...
    data['debug-mode-id'] = parser.get("debug", "debug-mode")
    data['enable-today-id'] = parser.get("debug", "enable-today")
    data['enable-tomorrow-id'] = parser.get("debug", "enable-tomorrow")
    data['enable-both-id'] = parser.get("debug", "enable-both")
...
def get_garbage_status():
  ...
  if garbage_schedule['debug-mode-id'] == "TRUE":
    if garbage_schedule['enable-today-id'] == "TRUE":
      print("DEBUG: Turning on Todays garbage")
    ...
    if garbage_schedule['enable-tomorrow-id'] == "TRUE":
      print("DEBUG: Turning on Tomorrow's garbage")
    ...
    if garbage_schedule['enable-both-id'] == "TRUE":
      print("DEBUG: Turning on Today & Tomorrow's garbage")
